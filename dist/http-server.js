...
class GHLMCPHttpServer {
     ...
     */
    setupExpress() {
        // Enable CORS for all MCP clients
    ...
    setupRoutes() {
        ...
        this.app.post('/mcp', async (req, res) => {
            ...
            try {
                ...
                console.error('[GHL MCP HTTP] Streamable HTTP POST error:', error);
        ...
        // ─────────────────────────────────────────────────────────────────
        // Legacy SSE endpoint — kept for ChatGPT compatibility
        // ─────────────────────────────────────────────────────────────────
...
// Module-level instance — used by Vercel serverless export and local dev
const _serverInstance = new GHLMCPHttpServer();
// Export Express app so Vercel can call it as a serverless handler
exports.default = _serverInstance.app;
// Only bind a port when running outside Vercel (local dev / Railway / etc.)
if (!process.env.VERCEL) {
    setupGracefulShutdown();
    _serverInstance.start().catch((error) => {
